...
powyzej.5tys <- covid %>%
  filter(data==max(data), liczba.zachorowan>5000)%>%
...
ggplot()+
  ...
  ggtitle("Ilość dni jaka upłynęła między przekroczeniem 425 i  5000 tysięcy przypadków zakażenia")+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5))

ggplot()+
  geom_boxplot(data=roznica, aes(y=ilosc.dni))
...
ilosc.za.tydzien <- round(bsts::GeometricSequence(10, 425, srednia.4dni),0)
