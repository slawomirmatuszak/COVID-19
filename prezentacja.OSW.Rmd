...
Wedug danych Ministerstwa Zdrowia  z `r unique(format(a$data2, "%d"))` `r if_else(unique(format(a$data2, "%B"))=="marzec", paste("marca"),if_else(unique(format(a$data2, "%B"))=="maj", paste("maja"), paste("kwietnia"))) ` `r unique(format(a$data2, "%Y"))` z godziny 9.00 od początku epidemii na Ukrainie zanotowano w sumie **`r as.integer(sum(a$liczba))`** przypadków zarażenia wirusem SARS-CoV-2 , z czego **`r sum(b$liczba)`** osób zmarło i **`r sum(c$liczba)`** się wyleczyło.  
...
#png("./wykresy/odsetek wyleczonych.aktywni.wyleczeni.20.maja.png", units="in", width=10, height=10, res=600)
...
```
```{r include=F}
png("./wykresy/odsetek wyleczonych.aktywni.wyleczeni.20.maja.porownanie.png", units="in", width=20, height=10, res=300)
